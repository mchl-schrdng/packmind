...
describe("pricing", () => {
  it("maps model families to rates", () => {
    expect(rateFor("claude-opus-4-8").outputPerMTok).toBe(75);
    expect(rateFor("some-sonnet-thing").inputPerMTok).toBe(3);
    expect(rateFor("unknown").inputPerMTok).toBe(15); // falls back to opus tier
  });
  it("computes dollar cost from tokens", () => {
    expect(inputCost("claude-opus-4-8", 1_000_000)).toBeCloseTo(15, 6);
    expect(outputCost("claude-opus-4-8", 1_000_000)).toBeCloseTo(75, 6);
