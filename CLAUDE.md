...
- `compress/` - `store.ts`: content-addressed store for shelved non-source blobs
  (`store`/`retrieve`/`compact`), backing the `compress`/`retrieve` MCP tools.
- `guard/` - `secrets.ts`, `path-guard.ts`, `policy.ts`.
- `adapters/claude-code.ts` - registers hooks + the MCP server into a project.
- `dashboard/` - loopback web dashboard: `server.ts` + `config-api.ts`; the HTML
  lives at `templates/dashboard.html` (served via `TEMPLATES_DIR`).
