...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Record a bug and its fix so it is never re-investigated.",
    inputSchema: {
      type: "object",
      properties: {
        error: { type: "string" },
    ...
    try {
      switch (req.params.name) {
        ...
        case "record_solution":
          return text(toolRecordSolution(ctx, { error: String(a.error ?? ""), cause: a.cause, fix: a.fix, tags: a.tags }));
        case "project_map":
